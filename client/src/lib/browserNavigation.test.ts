...
describe("appendNavigationRoute", () => {
  it("replaces a same-address Chromium route rather than creating a fallback mode", () => {
    const route = { url: "https://example.com/article", title: "example.com", mode: "web" as const };
    const result = appendNavigationRoute([{ url: "about:blank", mode: "search" }, route], 1, { ...route, title: "example.com · 已更新" });
    expect(result.historyIndex).toBe(1);
    expect(result.history).toEqual([{ url: "about:blank", mode: "search" }, { ...route, title: "example.com · 已更新" }]);
  });
  ...
  it("keeps successive site links in one current Chromium-tab path for back navigation", () => {
    const home = { url: "https://news.example.com/home", title: "新闻首页", mode: "web" as const };
    const article = { url: "https://news.example.com/article/42", title: "深度报道", mode: "web" as const };
    const video = { url: "https://news.example.com/video/9", title: "现场视频", mode: "web" as const };
  ...
  it("retains a direct media URL as a normal webview navigation", () => {
    const route = { url: "https://example.com/live.m3u8", mode: "web" as const };
